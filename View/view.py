...
class ScraperView:
    ...
    def get_references_data(self):
        ...
        for row_entries in self.rows:
            ...
            # Nouvelle ligne : on récupère via StringVar
            type_str  = row_entries[4].type_var.get().strip()

            # Conversion des numériques (inchangée)
            try: prix = float(prix_str)
            except: prix = None
            try: quantite = int(qte_str)
            except: quantite = None
            try: poids = float(poids_str)
            except: poids = None
    ...
    def add_product_headers(self):
        ...
        headers = ["Référence", "Prix", "Quantité", "Poids", "Type"]
    ...
    def add_product_row(self, row_index):
        """Ajoute une nouvelle ligne d'entrée pour un produit, avec OptionMenu pour le type."""
        row_entries = []
        # Colonnes Réf, Prix, Quantité, Poids (inchangées)
        for j in range(4):
        ...
        # Colonne Type : OptionMenu (fait office de bouton + menu)
        type_var = tk.StringVar(value="")  
        options = ["Techno / Electro", "Drum & Bass / Jungle / Dubstep", "House / Deep House", "Hardtek / Acidcore / Tribe", "Hardcore / Gabber", "Minimal / Micro House"]
        opt = tk.OptionMenu(self.frame_lignes, type_var, *options)
        opt.config(width=12)  # Ajuste la largeur si besoin
        opt.grid(row=row_index, column=4, padx=5, pady=2)
        opt.default_bg = opt.cget('bg')
        # Re-validation à chaque changement de sélection
        type_var.trace_add("write", lambda *args: self.controller.validate_all_inputs())
        # On attache la var au widget pour la récupérer ensuite
        opt.type_var = type_var
